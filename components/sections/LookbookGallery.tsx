  ...
  return (
    <section className="py-8 sm:py-16 border-t border-[#E5E0D8] w-full">
      ...
      <motion.div
        ...
        className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 sm:gap-4 mb-6 sm:mb-10"
      >
        <div>
      ...
      {/* Grid: 1 column on mobile, 2 on sm, 3 on md */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 sm:gap-6">
        {LOOKBOOK_ITEMS.map((item, idx) => (
          <motion.div
            ...
            className="group relative overflow-hidden rounded-2xl sm:rounded-3xl bg-[#F5F2ED] border border-[#E5E0D8] aspect-[3/4] flex flex-col justify-end p-5 sm:p-6 hover:shadow-[0_12px_30px_rgba(40,30,20,0.09)] transition-all duration-300 w-full"
          >
            <Image
              src={item.image}
              alt={item.title}
            ...
            <div className="absolute top-4 left-4 z-10 flex items-center gap-1.5 rounded-full bg-white/95 px-3 py-1 text-xs font-bold text-[#1E1D1B] shadow-sm border border-[#E5E0D8] backdrop-blur-xs">
